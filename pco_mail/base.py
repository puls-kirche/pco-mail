...
def _get_names(auth) -> dict:
    names = {}
    response = requests.get(PCO_URL + "/people/v2/people?per_page=200",
...
def _get_teams(auth) -> dict:
    teams = {}
    response = requests.get(PCO_URL + "/services/v2/teams" +
...
def _get_assignements(auth) -> dict:
    assignements = []
    response = requests.get(PCO_URL + "/services/v2/teams?per_page=500" + 
...
def _get_plans(auth) -> dict:
    plans = {}
    response = requests.get(PCO_URL + "/services/v2/series?per_page=100",
                            auth=auth, timeout=1000)
    res = json.loads(response.text)
    for nested_array in res['data']:
        identifier = nested_array['id']
        ...
        for nested_plan in res['data']:
            ...
            plans[plan_id] = {"series_id": identifier,
                              "series": series_title,
                              "date": plan_date,
                              "plan": plan_title}
